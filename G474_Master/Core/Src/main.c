...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
	...
	if (huart->Instance == huart3.Instance)	//ESP data
		...
		for (int i = 0; i < 5; i++) {
			sprintf(serial_output_buffer + strlen(serial_output_buffer),
					"%02X ", uart3_rx_data[i]);
		}
		Serial_Print();
	...
	if (huart->Instance == huart4.Instance)	//HMI data
		...
		if (uart4_rx_data[0] > 0x29 && uart4_rx_data[0] < 0x60) {
			...
			case '0':
				...
				Master_Tx_SPDO_Data.current_value =
						myEV.charging_current_request;
				break;
			case '1':
			...
			case '3':
				...
				Master_Tx_SPDO_Data.current_value = myHMI.current
						/ can_node_number;
				break;
			}
			...
			switch (myHMI.cable) {
			case '0':
				Master_Tx_SPDO_Data.mode = CHARGING_PLUG_1;
				break;
			case '1':
				Master_Tx_SPDO_Data.mode = CHARGING_PLUG_2;
				break;
			}
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t RxFifo0ITs) {
	if ((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != RESET) {
		if (HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO0,
				&CAN_Master_Rx_Header, CAN_Master_Rx_Data) != HAL_OK) {
			/* Reception Error */
			Error_Handler();
		}
		...
		if (list_node_available[id] != ID_IN_USED) {
			...
			sprintf(serial_output_buffer, "Node 0x%02X reconnect vao mang ",
					id);
			Serial_Print();
		}
		...
		if (Master_Rx_SPDO_Data.state == CHARGING_INIT) //có slave mới tham gia mang
		{
			...
			{
				sprintf(serial_output_buffer,
						"Co node moi tham gia, ID mong muon= 0x%02X ",
						Master_Rx_SPDO_Data.slaveID);
				Serial_Print();
				...
				if (list_node_available[desireID] == ID_NOT_USED) {
					...
					HAL_FDCAN_AddMessageToTxFifoQ(hfdcan, &CAN_Master_Tx_Header,
							CAN_Master_Tx_Data);
					//send message to uart3
					sprintf(serial_output_buffer, "Node moi co ID= 0x%02X ",
							Master_Tx_SPDO_Data.slaveID);
					Serial_Print();

					//write list id to flash
					uint64_t datawwritetoflash[Maximum_number_of_IDs / 8];
					uint8_array_to_uint64_array_big_endian(list_node_available,
					Maximum_number_of_IDs / 8, datawwritetoflash);
					writeFlash(datawwritetoflash, Maximum_number_of_IDs / 8,
							127, FLASH_BANK_1);
					...
					while (i < Maximum_number_of_IDs) {
						if (list_node_available[i] == ID_NOT_USED) {
							...
							Master_Tx_SPDO_Data.state = CHARGING_INIT; //tam thoi thay doi trang thai master sang khoi tao slave
							CAN_Master_Tx_Data[0] = CHARGING_SLAVE_ID; //tam thoi thay doi dia chi dich toi slave moi khoi tao
							PDO_set_data_frame(CAN_Master_Tx_Data,
									Master_Tx_SPDO_Data);
							HAL_FDCAN_AddMessageToTxFifoQ(hfdcan,
									&CAN_Master_Tx_Header, CAN_Master_Tx_Data);

							uint64_t datawwritetoflash[Maximum_number_of_IDs / 8];
							uint8_array_to_uint64_array_big_endian(
									list_node_available,
									Maximum_number_of_IDs / 8,
									datawwritetoflash);
							writeFlash(datawwritetoflash,
							Maximum_number_of_IDs / 8, 127,
							FLASH_BANK_1);
						...
						if (i == Maximum_number_of_IDs) {
							sprintf(serial_output_buffer,
									"Khong co ID phu hop cho node moi nay");
							Serial_Print();
						}
					}
				}
...
int main(void) {
	...
	HAL_FDCAN_GetRxMessage(&hfdcan2, FDCAN_RX_FIFO0, &CAN_Master_Rx_Header,
			CAN_Master_Rx_Data);
	...
	uint64_array_to_uint8_array_big_endian(datareadfromflash,
	Maximum_number_of_IDs / 8, list_node_available);
	...
	for (int i = 2; i < Maximum_number_of_IDs; i++)
		list_node_update[i] = 0; //the last time node communicated with CIMS, updated when node communicate
	for (int i = 2; i < Maximum_number_of_IDs; i++)
		previoustick_slave[i] = 0; //time to check how long node had disconnected with CIMS, updated by timer
...
//		TIM1->CCR1=duty;	//duty
		...
		if ((currenttick - previoustick_SPI) >= TPDO_Event_Time * 10) {
			...
			case INITIALIZATION + STATE_B + PARAMETER_EXCHANGE + REQUEST:
				...
				HMI_Compose_Pre_Charge_Parm(HMI_CAR_MODEL,
						myEV.control_protocol_number);
				HMI_Print();
				HMI_Compose_Pre_Charge_Parm(HMI_BATTERY_CAPACITY,
						myEV.rate_capacity_battery);
				HMI_Print();
				HMI_Compose_Pre_Charge_Parm(HMI_CURRENT_BATTERY,
						myEV.current_battery);
				HMI_Print();
				HMI_Compose_Pre_Charge_Parm(HMI_BATTERY_VOLTAGE,
						myEV.max_battery_voltage);
				HMI_Print();
				HMI_Compose_Pre_Charge_Parm(HMI_CHARGING_TIME,
						myEV.max_charging_time);
				HMI_Print();
				break;
...
//				HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan2, &CAN_Master_Tx_Header, CAN_Master_Tx_Data);
					for (int i = 0x02; i < (0x02 + can_node_number); i++) {
						...
						Master_Tx_SPDO_Data.type_of_value =
						CHARGING_DATA_CURRENT;
						PDO_set_data_frame(CAN_Master_Tx_Data,
								Master_Tx_SPDO_Data);
				...
				} else		//setting done, request data
				{
					...
					UpdateCANMasterTxDesID(0x02, can_node_number + 1,
							&Master_Tx_SPDO_Data, CAN_Master_Tx_Data);
					PDO_set_data_frame(CAN_Master_Tx_Data, Master_Tx_SPDO_Data);
					HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan2,
							&CAN_Master_Tx_Header, CAN_Master_Tx_Data);
				}
			...
			if (EVstate
					== (ENERGY_TRANSFER + STATE_C + CURRENT_SUPPRESSION
							+ CONFIRM))	//setting ve 0, current suppresion CNF
					{
				if (can_setting_confirm_flag < can_node_number * 4)	//setting CAN
						{
					Master_Tx_SPDO_Data.function = CHARGING_SETTING;
...
//				HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan2, &CAN_Master_Tx_Header, CAN_Master_Tx_Data);
			...
			for (int i = 2; i < Maximum_number_of_IDs; i++)	//kiem tra cac node ID co mat ket noi khong
				...
				if (list_node_available[i] == ID_IN_USED)//ID dang danh dau co ket noi
				{
					...
					if (previoustick_slave[i]
							- list_node_update[i]>Maximum_time_not_update_of_IDs)//NOT UPDATED for more than 5000ms =5s
							{
						list_node_available[i] = ID_DISCONNECTED;//id da mat ket noi, cho ket noi lại
						Reset_Data(&Slave_Data[i - 2]);				//reset data
						sprintf(serial_output_buffer,
								"Node ID= %d da mat ket noi ", i);
						Serial_Print();
						uint64_t datawwritetoflash[Maximum_number_of_IDs / 8];
						uint8_array_to_uint64_array_big_endian(
								list_node_available, Maximum_number_of_IDs / 8,
								datawwritetoflash);
						writeFlash(datawwritetoflash, Maximum_number_of_IDs / 8,
								127, FLASH_BANK_1);
					}
			...
			int count = 0;
			for (int i = 2; i < Maximum_number_of_IDs; i++) {
				if (list_node_available[i] == ID_IN_USED)
					count++;
			}
		...
		if ((currenttick - previoustick_HMI) >= TPDO_Event_Time * 5)//timer 500ms cho HMI. gui realtime data
			...
			if ((EVstate
					...
					|| (EVstate
							== (ENERGY_TRANSFER + STATE_C + CURRENT_SUPPRESSION
									+ CONFIRM)))//SPI CCC REQ, SPI Current suppression REQ, SPI Current suppression CNF
					{
				...
				if (check == 0)	//khong co cai nao bi loi
						{
					HMI_Compose_Realtime_Data(HMI_VOLTAGE, Voltage);
					HMI_Print();
					HMI_Compose_Realtime_Data(HMI_CURRENT, Current);
					HMI_Print();
					HMI_Compose_Realtime_Data(HMI_TEMP, Temperature);
					HMI_Print();
					HMI_Compose_Realtime_Data(HMI_IREF,
							myEV.charging_current_request);
					HMI_Print();
				}
				...
				for (int i = 0; i < can_node_number; i++) {
					if (Slave_Data[i].state != CHARGING_ON
							&& can_setting_confirm_flag
									>= can_node_number * 4) {
						HMI_Compose_Status(HMI_ERROR_1);
						HMI_Print();
					}
				}
