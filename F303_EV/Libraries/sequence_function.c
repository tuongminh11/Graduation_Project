...
void PEF_Evaluate_Exchange_Data(){
	...
	case INITIALIZATION + STATE_B + COMMUNICATION_INIT + REQUEST:   //EVSE xử lý
		PEF_Handle_Initialization_Req();
		break;
	case INITIALIZATION + STATE_B + COMMUNICATION_INIT + CONFIRM:  // EV xử lý
		PEF_Handle_Initialization_Cnf();
		break;
	case INITIALIZATION + STATE_B + PARAMETER_EXCHANGE + REQUEST:  //EVSE xử lý
		PEF_Handle_Parameter_Exchange_Req();
		break;
	case INITIALIZATION + STATE_B + PARAMETER_EXCHANGE + RESPONSE: //EV xử lý
		PEF_Handle_Parameter_Exchange_Res();
		break;
	case INITIALIZATION + STATE_B + PARAMETER_EXCHANGE + CONFIRM:  //EVSE xử lý, dừng ở EVSE
		PEF_Handle_Parameter_Exchange_Cnf();
		break;
	case INITIALIZATION + STATE_B + EVSE_CONNECTOR_LOCK + REQUEST: // EV gửi REQ sang EVSE, EVSE xử lý gói REQ và hiện connected
		PEF_Handle_Connector_Lock_Req();
		break;
	case INITIALIZATION + STATE_B + EVSE_CONNECTOR_LOCK + CONFIRM:  //EV xử lý, dừng ở đây
		PEF_Handle_Connector_Lock_Cnf();
		break;
	case ENERGY_TRANSFER + STATE_C + EV_CONTACTOR_CLOSE + REQUEST: // Bấm nút start, EVSE gửi REQ sang EV
	...
	case ENERGY_TRANSFER + STATE_C + CHARGING_CURRENT_DEMAND + RESPONSE:
		...
		break;
	case ENERGY_TRANSFER + STATE_C + CURRENT_SUPPRESSION + REQUEST:  // EV gửi
		PEF_Handle_Current_Suppression_Req();
		break;
	case ENERGY_TRANSFER + STATE_C + CURRENT_SUPPRESSION + RESPONSE: // EVSE gửi
		PEF_Handle_Current_Suppression_Res();
		break;
	case ENERGY_TRANSFER + STATE_C + CURRENT_SUPPRESSION + CONFIRM: // EV gửi
